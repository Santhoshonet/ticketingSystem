class Ticket < ActiveRecord::Base
  ...
  def self.timeline_opened_tickets(from_date, to_date)
    ...
    self.count(:group => 'date(tickets.created_at)', :conditions => ['tickets.created_at >= ? and tickets.created_at < ?', from_date, to_date], :order => 'date_tickets_created_at')
  end
  ...
  def self.timeline_closed_tickets(from_date, to_date)
    ...
    self.count(:group => 'date(tickets.closed_at)', :conditions => ['tickets.closed_at >= ? and tickets.closed_at < ?', from_date, to_date], :order => 'date_tickets_closed_at')
  end
